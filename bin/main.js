...
HttpExtensions.promise = function(h,request,err) {
	if(request == null) request = false;
	var p = new promhx.Promise(err);
	h.onError = $bind(p,p.reject);
	var set = false;
	h.onData = function(x) {
		if(!set) p.resolve(x); else set = true;
	};
	h.request(request);
	console.log("p=" + Std.string(p));
	return p;
}
var Std = function() { }
Std.__name__ = true;
